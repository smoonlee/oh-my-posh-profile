...
Invoke-WebRequest -Uri 'https://psg-prod-eastus.azureedge.net/packages/psreadline.2.2.0-beta1.nupkg' -OutFile "$TempPath\psreadline.2.2.0-beta1.nupkg"

# Extract NuGet Pkg
Move-Item -Path "$TempPath\psreadline.2.2.0-beta1.nupkg" -Destination "$TempPath\psreadline.2.2.0-beta1.zip" -Force | Out-Null
Expand-Archive -Path "$TempPath\psreadline.2.2.0-beta1.zip" -DestinationPath "$TempPath\2.2.0" -Force | Out-Null
Remove-Item -Path "$TempPath\psreadline.2.2.0-beta1.zip"
